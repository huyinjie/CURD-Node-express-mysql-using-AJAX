...
// var curut2 = router.route('/login_page/:member_id');

/*------------------------------------------------------
route.all is extremely useful. you can use it to do
stuffs for specific routes. for example you need to do
a validation everytime route /api/login_page/:member_id it hit.

remove curut2.all() if you dont want it
------------------------------------------------------*/
// curut2.all('/login_page/:member_id', function (req, res, next) {
...
//get data to update
router.get('/login_page/:member_id', function (req, res, next) {
	if (req.session.loggedin) {
		...
		req.getConnection(function (err, conn) {
			...
			var query = conn.query(sql, [member_id], function (err, result) {
				...
				res.render('login_edit', { title: "Edit login_page", data: result });
			});
		});
	}
});
...
// 🎉 Update data
router.put('/login_page/:member_id', function (req, res, next) {
	var member_id = req.params.member_id;
...
//delete data
router.delete('/login_page/:member_id', function (req, res, next) {
	var member_id = req.params.member_id;
	req.getConnection(function (err, conn) {
		if (err) return next("Cannot Connect");
